...
void main() {
  ...
  test('unseen remote identities are inserted as new people', () async {
    ...
    final id = index.identityForName('jordan');
    expect(id, isNotNull);
  ...
  test('the side with more samples supplies the canonical name', () async {
    // Local "Mom" has 1 sample; remote "Kim" has 5 and lists Mom as alias.
    ...
    expect(index.identityForName('Kim'), isNotNull);
    expect(index.identityForName('Mom')!.name, 'Kim');
    // The face stored under the old canonical name was renamed.
    expect(index.identityForName('Kim')!.allNames, contains('Mom'));
  });
  ...
  test('bestFaces prefers confirmed faces over large low-confidence ones',
    ...
    final wrong = entryWithFace('photo-wrong', emb(2), name: 'Jordan');
    ...
    final best = index.bestFaces()['Jordan'];
